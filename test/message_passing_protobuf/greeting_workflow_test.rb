  ...
  class GreetingWorkflowTest < Test
    ...
    def test_queries
      with_worker_running do |client, worker|
        ...
        assert_equal %w[chinese english], handle.query(GreetingWorkflow.get_state, { include_unsupported: false })
        assert_equal CallGreetingService.greetings.keys.map(&:to_s).sort,
                     handle.query(GreetingWorkflow.get_state, { include_unsupported: true })
      end
    end
